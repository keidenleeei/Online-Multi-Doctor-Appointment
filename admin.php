...
if (isset($_GET['updated'])) {
    $alert_message = "Doctor profile updated successfully.";
    $alert_type = "success";
}
...
if (isset($_GET['delete_doctor'])) {
    ...
    if ($find_stmt) {
        ...
        if ($find_result->num_rows > 0) {
            ...
            $schedule_count_stmt = $conn->prepare("SELECT COUNT(*) AS total FROM schedules WHERE doctor_id = ?");
            $appointment_count_stmt = $conn->prepare("SELECT COUNT(*) AS total FROM appointments WHERE doctor_id = ? AND status <> 'Cancelled'");

            $has_links = false;
            if ($schedule_count_stmt && $appointment_count_stmt) {
                $schedule_count_stmt->bind_param("i", $doctor_id);
                $schedule_count_stmt->execute();
                $schedule_row = $schedule_count_stmt->get_result()->fetch_assoc();

                $appointment_count_stmt->bind_param("i", $doctor_id);
                $appointment_count_stmt->execute();
                $appointment_row = $appointment_count_stmt->get_result()->fetch_assoc();

                $has_links = ((int)$schedule_row['total'] > 0) || ((int)$appointment_row['total'] > 0);
                $schedule_count_stmt->close();
                $appointment_count_stmt->close();
            }

            if ($has_links) {
                $alert_message = "This doctor still has schedules or appointments, so deletion is blocked.";
                $alert_type = "error";
            } else {
...
if (isset($_GET['confirm'])) {
    ...
    $confirm_stmt = $conn->prepare("UPDATE appointments SET status = 'Confirmed' WHERE appointment_id = ? AND status = 'Pending'");
    if ($confirm_stmt) {
...
if (isset($_GET['cancel'])) {
    ...
    $cancel_stmt = $conn->prepare("UPDATE appointments SET status = 'Cancelled' WHERE appointment_id = ? AND status = 'Pending'");
    if ($cancel_stmt) {
...
<head>
  ...
  <title>Admin Panel - MedLink Appointment System</title>
...
<body class="page-bg">
  ...
  <main class="wrap page-main">
    <div style="margin-bottom: 2rem;">
      ...
      <p style="color: var(--muted); margin-top: 0.25rem;">Monitor schedules, manage appointments, and configure doctor accounts.</p>
    </div>

    <?php if ($alert_message !== "") { ?>
      <div style="padding: 12px 16px; border-radius: 8px; margin-bottom: 2rem; font-size: 0.95rem; font-weight: 500; <?php echo $alert_type === 'success' ? 'color: #065f46; background: #d1fae5; border: 1px solid #a7f3d0;' : 'color: #b91c1c; background: #fee2e2; border: 1px solid #fca5a5;'; ?>">
        <?php echo $alert_type === 'success' ? '+' : '!'; ?> <?php echo htmlspecialchars($alert_message); ?>
      </div>
    <?php } ?>
                ...
                <td>
                  <?php if ($status === "Pending") { ?>
                    <div style="display: flex; gap: 0.4rem;">
                      <a href="admin.php?confirm=<?php echo (int)$row['appointment_id']; ?>" class="btn primary" style="min-height: 2.2rem; padding: 0.25rem 0.85rem; font-size: 0.82rem; border-radius: 6px;">Approve</a>
                      <a href="admin.php?cancel=<?php echo (int)$row['appointment_id']; ?>" class="btn secondary" style="min-height: 2.2rem; padding: 0.25rem 0.85rem; font-size: 0.82rem; border-radius: 6px; color: var(--danger); border-color: rgba(239, 68, 68, 0.2);" onclick="return confirm('Cancel this appointment?')">Cancel</a>
                    </div>
                  <?php } else { ?>
            ...
            <?php if ($doctors_result && mysqli_num_rows($doctors_result) > 0) { ?>
              <?php while ($doctor = mysqli_fetch_assoc($doctors_result)) { ?>
                <tr>
                  ...
                  <td>
                    <div style="display: flex; gap: 0.4rem;">
                      <a href="edit_doctor.php?doctor_id=<?php echo (int)$doctor['doctor_id']; ?>" class="btn primary" style="min-height: 2rem; padding: 0.2rem 0.65rem; font-size: 0.8rem; border-radius: 6px;">Edit</a>
                      <a href="admin.php?delete_doctor=<?php echo (int)$doctor['doctor_id']; ?>" class="btn secondary" style="min-height: 2rem; padding: 0.2rem 0.65rem; font-size: 0.8rem; border-radius: 6px; color: var(--danger); border-color: rgba(239, 68, 68, 0.2);" onclick="return confirm('Are you sure you want to delete this doctor profile? This will remove their user account as well.')">Delete</a>
                    </div>
                  </td>
                </tr>
              <?php } ?>
      ...
      <section class="card glass-card">
        ...
        <form method="POST" action="admin.php">
          ...
          <label class="field">
            ...
            <input type="text" name="specialization" placeholder="General Practice" required />
          </label>
          ...
          <button class="btn primary" type="submit" name="add_doctor" style="width: 100%; border-radius: 12px; margin-top: 0.5rem;">Add Doctor</button>
        </form>
      </section>
    </div>
  </main>
